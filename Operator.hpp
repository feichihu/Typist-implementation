...
{
    public:
        Operator(std::string word, int duration, std::string t);//init, where duration is hardcoded or given by Emma/Fitts
        virtual ~Operator();
        ...
        int update();//updata data related to critical path
        int time_elapsed();
    ...
    protected:
        ...
        int highest_duration=0;
        int Real_duration = 340;
        std::string chunck;
